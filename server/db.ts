...
import { InsertUser, users, orders, leadLists, orderDownloads, Order, InsertOrder, LeadList } from "../drizzle/schema";
...
export async function createLeadList(data: Omit<LeadList, 'id' | 'createdAt' | 'updatedAt'>) {
  const db = await getDb();
...
export async function getOrderByStripePaymentIntentId(stripePaymentIntentId: string) {
  const db = await getDb();
  if (!db) return undefined;
  const result = await db.select().from(orders).where(eq(orders.stripePaymentIntentId, stripePaymentIntentId)).limit(1);
  return result.length > 0 ? result[0] : undefined;
}
...
export async function getOrdersForDay(daysAgo: number) {
  const db = await getDb();
  if (!db) return [];
  const startDate = new Date();
  startDate.setDate(startDate.getDate() - daysAgo);
  return db.select().from(orders).where(and(eq(orders.status, 'completed'), gte(orders.createdAt, startDate)));
}

export async function recordOrderDownload(orderId: number) {
  const db = await getDb();
  if (!db) throw new Error("Database not available");
  await db.insert(orderDownloads).values({ orderId });
